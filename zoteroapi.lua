...
function API.syncAllItems()
	...
	local collections_url = ("https://api.zotero.org/users/%s/collections?since=%s&includeTrashed=true"):format(
		user_id,
		since
	)
	...
	local r, e = API.fetchCollectionPaginated(items_url, headers, function(partial_entries)
		...
		for i = 1, #partial_entries do
			print("Processing item " .. i .. " of " .. #partial_entries)
			...
			print("Item " .. key .. " processed, type: " .. item.data.itemType)
		end
	end)
	if e ~= nil then
		return e
...
function API.downloadAndGetPath(key, download_callback)
	...
	-- end
	if item.data.itemType ~= "attachment" and item.data.itemType ~= "webpage" then
		return nil, "Error: this item is not an attachment"
	end
...
function API.displayCollection(key)
	...
	for k, collection in pairs(collections) do
		if
			(key == nil and collection.data.parentCollection == false)
			or (key ~= nil and collection.data.parentCollection == key)
		then
			table.insert(result, {
				["key"] = k,
	...
	for k, item in pairs(items) do
		print(
			"Checking item "
				...
				.. item.data.itemType
				.. " and content type "
				.. (item.data.contentType or "nil")
		)
		if
			(item.data.itemType == "attachment" or item.data.itemType == "webpage")
		...
		then
			if item.data.parentItem ~= nil then
				...
				if item.data.collections ~= nil and table_contains(item.data.collections, key) then
					table.insert(collectionItems, {
						["key"] = k,
...
function API.localTimezone(timestamp) end
