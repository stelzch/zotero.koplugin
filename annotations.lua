...
function Annotations.colorKOReaderToZotero(color_name)
	return K2Z_COLORS[string.lower(color_name)] or K2Z_COLORS["yellow"]
end
...
function Annotations.createAnnotations(file_path, key, creation_callback)
	...
	if page_dimensions == nil then
		logger.err(("Zotero: Skipping '%s' because page dimensions can not be determined. relevant pages where %s")
			:format(file_path, JSON.encode(pages)))
		return nil
	end
